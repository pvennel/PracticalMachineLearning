...
4. So we now have 19622 observations with 51 variables.
...
I have used the following 3 model types:
...
First we fit the 3 models using the training dataset. Then we run the predictive model based on the test data. Finally we compare our prediction with **classe** variable to verify accuracy, using Confusion Matrix. 

After running the Confusion Matrix on the models, we got some more clarity on the mode. Below are some of the key metrics for comparing the model for effectiveness: 

No | Model | Accuracy |  95% CI  | Kappa | P-Value [Acc > NIR]
---|-------|----------|--------|-------|-------------------
1 | lda | 70.48% |  (0.6930, 0.7165)  | 62.65% | < 2.2e-16
2 | rpart | 48.90% |  (0.4762, 0.5019)  |33.11% | < 2.2e-16
3 | rf | 99.44% |  (0.9921, 0.9961)  | 99.29% | < 2.2e-16

Clearly from the above 3 models, we see that Random Forest is more effective and accurate.

We will now directly use the randomForest package so we can configure the parameters. Building models using **train** takes more time and has less options to tune. 

##Final Model Fitting
I will now use **randomForest** package. See the **confusionMatrix()** result <mark>(in Appendix A4)</mark>. We now get accuracy of **99.64%**. Now looking at the **varImplPlot()** result <mark>(See Appendix A4 for details)</mark>, I could identify which variables are important. This helps us in reducing the variables without reducing the quality of the model.        
...
```{r}
confusionMatrix(plda, testing$classe)$overall
confusionMatrix(prpart, testing$classe)$overall
confusionMatrix(prf, testing$classe)$overall
```
...
modrfDirectNew <- randomForest(classe ~ ., data=trainingNew, ntree=1000, keep.forest=TRUE, importance=TRUE)
```
